...
-   [The blunder](#sec-blunder): I asked a vague question with no benchmarking
...
This is the statistics-lesson part of *ARQ*. If you've made it this far you're a big dork and it's too late to go back. Welcome to after-school statistics club.

This week, we'll be looking at a really important fundamental idea that's often skipped right over when we think about statistics. You probably don't need to understand it to do your job, but I hope that it will equip you to think critically and carefully about statistics of all kinds. It's the importance of **models** in statistical testing.
...
![](images/clipboard-3914663401.jpeg){fig-alt="A picture of a hamster on a bicycle"}
...
Congratulations, you've just conducted your first statistical test, and you've done it by creating your first statistical model. Any attempt to 'prove' something with statistics uses - in one way or another - hamsters on tiny bicycles.
...
Functionally, what you're doing here is putting forward a hypothesis: This coin is a *freaky coin*. It comes up heads too often. This statement implies a comparison with a *normal coin*. You don't have a normal coin on you, but you don't need one, because you can create a mental model of a normal coin with almost no effort. Your mental model of a normal coin comes up heads about half the time. In ten flips, you'd expect to see five or six heads, maybe seven. But *eight*? That's too many. That's very improbable. So you reject the hypothesis that this coin is normal, and conclude that it is freaky, because you imagined a normal coin and it didn't come up heads that many times.

None of that is particularly well formalised or conclusive, but none of it is categorically different from 'proper' statistical testing. Let's do some proper testing now by doing ten flips one million times, using the computer:

``` r
# ~~ R ~~
# We will simulate 1,000,000 times 10 'normal' coin flips
n_tests <- 1e6 # (ie. 1,000,000)
pretend_flips <-
  rbinom(n = n_tests,    # one million tests,
         size = 10,  # ten flips,
...
proportion_8_heads <- number_of_times_we_got_8_heads_or_more/n_tests
